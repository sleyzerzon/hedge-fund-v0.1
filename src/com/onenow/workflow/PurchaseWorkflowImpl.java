...
public class PurchaseWorkflowImpl implements PurchaseWorkflow {
	...
	public void mainFlow() {
		...
		// get underlying
		Promise<List<Underlying>> underlyingPromise = getBroker().getUnderlying();
		List<Underlying> underlying = getUnderList(underlyingPromise);
		
		Promise<List<Investment>> mktInvPromise = getBroker().getInvestments(false); // overall market
		...
		expDate.setTime(1000000);
 		...
 		Investment call1 = getBest(underlying.get(0), InvType.CALL, expDate, 405.00);
 		Investment call2 = getBest(underlying.get(0), InvType.CALL, expDate, 400.00);
 		Investment put1 = getBest(underlying.get(0), InvType.PUT, expDate, 390.00);
 		Investment put2 = getBest(underlying.get(0), InvType.PUT, expDate, 385.00);
 		
 		// buy something
 		Trade tradeStock = new Trade(stock, TradeType.BUY, 50, getPriceAsk(stock));
 		...
 		Transaction tx = new Transaction(tradeCall1, tradeCall2, tradePut1, tradePut2);
  		getBroker().addTrade(tx);
 		
 		Promise<List<Trade>> tradesPromise = getBroker().getTrades();
 		List<Trade> trades = getTrades(tradesPromise);
  		
		Promise<List<Investment>> myInvPromise = getBroker().getInvestments(true); 
	...
	@Asynchronous
	private List<Underlying> getUnderList(Promise<List<Underlying>> listPromise) {
	...
	@Asynchronous
	private Investment getBest(Underlying under, InvType invType) { // stock & spot/on-demand
	...
	@Asynchronous
	private Investment getBest(Underlying under, InvType invType, Date expDate, Double strike) { // options
		Investment inv = new Investment();
		try {
